...
from data_processing.scalers import apply_transformation_pipeline, apply_transformation_pipeline_raw
...
def compute_features(
    ...
    dataset = add_magneto_sphere_features(dataset)
    # Transform target
    ...
    dataset, fit_column_transforms = add_all_domain_features(dataset, column_transforms, fit_transform)

    return dataset, fit_target_transforms, fit_column_transforms


def add_magneto_sphere_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset, _ = _add_lag_based_feature(dataset, short_recovery_col, [4, 8])
    dataset, _ = _add_lag_based_feature(dataset, medium_recovery_col, [4, 8])
    dataset, _ = _add_lag_based_feature(dataset, long_recovery_col, [4, 8])
    # print(" >>>> Done adding Recovery State!")
   
    # Energy loading state
    dataset = add_energy_loading_state(dataset)
    dataset, _ = _add_lag_based_feature(dataset, ENERGY_LOADING_STATE_COL, [4, 8])
    # print(" >>>> Done adding Energy Loading State!")

    # Magnetotail loading state
    dataset = add_magnetotail_loading_state(dataset)
    dataset, _ = _add_lag_based_feature(dataset, MAGNETOTAIL_LOADING_STATE_COL, [4, 8])
    # print(" >>>> Done adding Magnetotail Loading State!")

    # Coupling efficiency state
    dataset = add_coupling_efficiency_state(dataset)
    dataset, _ = _add_lag_based_feature(dataset, COUPLING_EFFICIENCY_STATE, [4, 8])
...
def add_standard_solar(
    ...
    dataset, rolling_plasma_cols = _add_rolling_features(dataset, SW_SPEED_COL, window=18, overlap=0.3)
    dataset, rolling_temp_cols = _add_rolling_features(dataset, SW_TEMP_COL, window=18, overlap=0.3)
    dataset, rolling_density_cols = _add_rolling_features(dataset, SW_DENSITY_COL, window=18, overlap=0.3)
    dataset, rolling_beta_cols = _add_rolling_features(dataset, PLASMA_BETA_COL, window=18, overlap=0.3)
    ...
    for _col in rolling_plasma_cols + rolling_temp_cols + rolling_density_cols + rolling_beta_cols:
        dataset, _ = _add_lag_based_feature(dataset, _col, [8, 12])
    ...
    # Step 4: Scale the original features
    if column_transforms:
        for _col in [PLASMA_BETA_COL, SW_DENSITY_COL, SW_TEMP_COL, SW_SPEED_COL]:
            trans = column_transforms.get(_col)
            if trans:
                dataset, _fit_col_transforms = apply_transformation_pipeline(dataset, _col, trans, fit_transform)
                fit_col_transforms[_col] = _fit_col_transforms
    # print(" >>>> Done scaling solar features!")

    # Step 5: Lag the scaled originals
    dataset, plasma_cols = _add_lag_based_feature(dataset, PLASMA_BETA_COL, [8, 12])
    dataset, density_cols = _add_lag_based_feature(dataset, SW_DENSITY_COL, [8, 12])
    dataset, temp_cols = _add_lag_based_feature(dataset, SW_TEMP_COL, [8, 12])
    dataset, speed_cols = _add_lag_based_feature(dataset, SW_SPEED_COL, [8, 12])
...
def add_standard_magnetic(
    ...
    for _col in rolling_bz_cols + rolling_scalar_b_cols:
        dataset, _ = _add_lag_based_feature(dataset, _col, [4, 8])
    ...
    # Step 4: Scale the original features
    if column_transforms:
        for _col in [BZ_COL, BY_COL, SCALAR_B_COL]:
            trans = column_transforms.get(_col)
            if trans:
                dataset, _fit_col_transforms = apply_transformation_pipeline(dataset, _col, trans, fit_transform)
                fit_col_transforms[_col] = _fit_col_transforms
    # print(" >>>> Done scaling magnetic features!")

    # Step 5: Lag the scaled originals
    dataset, _ = _add_lag_based_feature(dataset, BZ_COL, [4, 8])
    dataset, _ = _add_lag_based_feature(dataset, BY_COL, [4])
    dataset, _ = _add_lag_based_feature(dataset, SCALAR_B_COL, [4, 8])
...
def add_standard_others(
    ...
    # Step 4: Scale the original features
    if column_transforms:
        for _col in [FLOW_PRESSURE_COL, E_FIELD_COL, ALPHA_PROTON_RATIO_COL]:
            trans = column_transforms.get(_col)
            if trans:
                dataset, _fit_col_transforms = apply_transformation_pipeline(dataset, _col, trans, fit_transform)
                fit_col_transforms[_col] = _fit_col_transforms
    # print(" >>>> Done scaling other features!")

    # Step 5: Lag the scaled originals
    dataset, _ = _add_lag_based_feature(dataset, FLOW_PRESSURE_COL, [4, 8])
    dataset, _ = _add_lag_based_feature(dataset, E_FIELD_COL, [4, 8])
    dataset, _ = _add_lag_based_feature(dataset, ALPHA_PROTON_RATIO_COL, [8, 12])
